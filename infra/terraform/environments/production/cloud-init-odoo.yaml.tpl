...
#                         (separate check from the blogs backup). GOL-855: this
#                         is the obs-stack OpenObserve JSON-ingest URL (basic-auth
#                         embedded), NOT Healthchecks.io (board 3a05a48b). Name
...
write_files:
  ...
  - path: /usr/local/bin/grove-odoo-backup.sh
    ...
    content: |
      ...
      #
      # GOL-855 (board decision 3a05a48b): the receiver is the self-hosted obs
      ...
      # alert fires to Discord (via Keep) when no heartbeat lands in ~25h. We POST
