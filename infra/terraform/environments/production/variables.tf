...
variable "hub_image_tag" {
  ...
  type        = string
  default     = "578b828b857cefa80b50551a9215c7c9bdcf2561"
...
# the module's "queue now, drain when keyed" contract. user_data is in
# ignore_changes (odoo.tf), so landing this scaffold does NOT touch the running
# prod droplet; activation rides a board-gated -target'ed rebuild (Josh).
variable "perenual_api_key" {
  description = "Perenual v2 API key for the prod plant-facts enrichment queue (GOL-2507, parent GOL-2383). services/plant_data/perenual.py reads os.environ PERENUAL_API_KEY; empty => PerenualProvider.configured is false and _cron_process_enrich_jobs() no-ops with jobs left queued. 1P: op://Goldberry Grove - Admin/AgenticOS Infra/perenual_api_key (Josh vaults the value; .env.op ref stays COMMENTED until the field exists). Budget is 100 calls/UTC day, enforced in the module via ir.config_parameter grove_headless.perenual_daily_budget — NOT here; a key SHARED with QA splits that one vendor quota across two databases (see .env.op). user_data input — droplet-REPLACE semantics like shippo_api_key, activation is Josh's -target'ed apply."
  type        = string
  sensitive   = true
  default     = ""
