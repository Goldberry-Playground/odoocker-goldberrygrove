...
write_files:
  ...
  - path: /etc/grove/.env
    ...
    content: |
      ...
      # Generated by the DEPLOYER (workflow or operator shell) and passed
      # through TF, NOT generated on-droplet like the other __*__ passwords.
      # Why: the deployer needs this value post-deploy to construct
      # QA_PORTAL_DATABASE_URL for Infisical, and the droplet firewall
      # scopes SSH to the admin IP only (Tier 1 hardening) -- GitHub
      # runners can NEVER ssh in to read it back. Discovered 2026-07-04
      # after every post-deploy Infisical push had failed since the
      # firewall landed; the earlier "slow boot" sentinel timeouts were
      # the same block, misdiagnosed.
      QA_PORTAL_PG_PASSWORD=${qa_portal_pg_password}
