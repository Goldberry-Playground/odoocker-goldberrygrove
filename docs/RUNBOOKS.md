...
**Severity:** critical (Odoo filestore) / warning (blogs) · **Likely impact:** the nightly off-box backup has not run in >25h. No data is lost *yet*, but the safety net has a hole — a droplet loss now would lose everything written since the last good backup.

> Dead-man's switch (GOL-855, board 3a05a48b). The prod backup scripts POST a `{job,stamp,...}` heartbeat to the obs-stack OpenObserve `backup_heartbeat` stream **only on success** (`set -euo pipefail` skips the heartbeat on any failure). The absence alert fires when no heartbeat for that job lands in ~25h. `job=odoo-filestore` (critical) covers customer product photos + `ir.attachment` binaries; `job=blogs` (warning) covers the Ghost MySQL dumps + content tarballs.

### Symptoms
- No `backup_heartbeat` row for the job in the last ~25h (the alert's own condition).
