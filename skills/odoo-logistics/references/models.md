...
`product.supplierinfo`, `uom.uom`, `uom.category`.

Inventory: `stock.quant`, `stock.move`, `stock.move.line`, `stock.picking`,
`stock.picking.type`, `stock.location`, `stock.warehouse`, `stock.lot`,
`stock.scrap`.
...
`product.pricelist.item`, `product.supplierinfo`, `stock.quant`, `stock.move`,
`stock.move.line`, `stock.picking`, `stock.lot`, `stock.scrap`,
`purchase.order`, `purchase.order.line`, `delivery.carrier`,
`stock.package.type`.
