...
echo "NOTE: the following are intentionally left in place across teardowns --"
echo "      they're idle without a droplet to attach to, and reusing them on"
echo "      the next qa-deploy is faster than recreating:"
echo "        - SSH keys (grove-qa-admin, grove-qa-deploy)"
echo "        - Firewall (grove-qa-fw)"
echo "        - Block-storage volume (grove-qa-caddy-data) -- holds Caddy's LE"
echo "          certs + ACME state. Critical to keep across teardowns to avoid"
echo "          Let's Encrypt's 5-certs-per-week-per-identifier rate limit."
echo "      To fully delete the volume (DESTRUCTIVE -- next deploy must re-do"
echo "      the LE cert dance and may hit rate limit if done frequently):"
echo "        curl -X DELETE -H \"Authorization: Bearer \$DIGITALOCEAN_TOKEN\" \\"
echo "          \"https://api.digitalocean.com/v2/volumes/<id-from-tf-output>\""
